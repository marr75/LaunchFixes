        ...
        try {
            ...
            if (list == null || list.Count == 0) {
                ...
                if (found == null || found.Length == 0) {
                    log.LogWarning(
                        "[VehicleStatsDumper] No LaunchVehicleType instances found (singleton list empty, fallback empty)."
                    );
                    return;
                }
            ...
            var header =
                $"{"ID",-40} {"Name",-28} {"Payload",10} {"FuelLoad",10} {"CostLaunch",12} {"ExhaustV",10} {"Reuse",7}";
            var rule = new string('-', header.Length);
            sb.AppendLine(rule);
