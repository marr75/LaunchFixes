...
static class CycDiag {
    ...
    // need no locking.
    static readonly Dictionary<string, string> LastSig = new();
    static readonly Dictionary<string, double> LastEmitTime = new();
    static readonly HashSet<string> Hit = new();
    ...
    internal static void Event(string site, string sig, string line) {
        if (!Enabled) { return; }
        if (LastSig.TryGetValue(site, out var prev) && prev == sig) { return; }
        LastSig[site] = sig;
        Log($"{site} {line}");
    }
    ...
    internal static void FirstHit(string site) {
        if (!Enabled || !Hit.Add(site)) { return; }
        Log($"FIRST-HIT {site}");
    }
    ...
    internal static string R(double v) =>
    ...
    internal static string Context(PMMissionParameter p) {
        string sc;
        try { sc = p.SC?.GetTypeSpaceCraft()?.ID ?? "noSC"; }
        catch { sc = "?SC"; }
        string lv;
        try { lv = p.LV?.GetLaunchVehicleType()?.ID ?? "none"; }
        catch { lv = "?LV"; }
