...
sealed class Configuration {
    ...
    public Configuration(ConfigFile c) {
        const string nearEmptyLaunchCostFloorDescription =
            "Minimum launch cost for an almost-empty launch vehicle, as a fraction of a full "
        ...
        const string launchVehiclePaysForAscentDescription =
            "When on, the launch vehicle is billed for your craft's launch burn and your craft keeps "
            + "a full tank on arrival. When off, your craft burns its own fuel to launch (vanilla). "
            + "Changes apply immediately, no restart needed.";
        LaunchVehiclePaysForAscent = c.Bind(
            "Balance",
            "LaunchVehiclePaysForAscent",
            true,
            launchVehiclePaysForAscentDescription
        );
        const string autoDumpVehicleStatsOnLoadDescription =
            "Writes every launch vehicle's stats to a file as soon as the game finishes loading them. "
            + "Only useful for reporting bugs or digging into the numbers yourself.";
        AutoDumpVehicleStatsOnLoad = c.Bind(
            "Debug",
            "AutoDumpVehicleStatsOnLoad",
            false,
            autoDumpVehicleStatsOnLoadDescription
        );
        const string dumpVehicleStatsHotkeyDescription =
            "Keyboard shortcut that dumps launch vehicle stats to a file on demand, regardless of the "
